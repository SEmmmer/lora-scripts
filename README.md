...
### 2. 阶段分辨率训练（支持 1024/2048 基准）

- 启用后会按基础分辨率自动拆分三阶段训练：
  - 基准 `1024,1024`：`512 -> 768 -> 1024`
  - 基准 `2048,2048`：`1024 -> 1536 -> 2048`
- 支持各阶段占比自定义（0%~100%，总和不大于 100%）。
...
- 当 `gradient_accumulation_steps > 1`：三阶段都保持与当前基础分辨率基准一致。


### 4. 采样与保存频率联动优化

- 阶段分辨率下，ckpt 与 sample 频率按基础分辨率缩放：
  - 基准 `1024`：`1024=x, 768=ceil(1.78x), 512=ceil(4x)`
  - 基准 `2048`：`2048=x, 1536=ceil(1.78x), 1024=ceil(4x)`
