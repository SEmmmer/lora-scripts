...
- 训练启动前会检查目标 `output_dir` 是否已有历史训练产物（ckpt / state / 训练目录）。
- 若检测到已有产物但未填写 `resume`，会直接提示并阻止启动，避免误覆盖或断点错接。
- 若填写了 `resume`，会校验其是否为有效 state 目录（需含 `train_state.json`）。
- 同时校验 `resume` 必须来自同一个 `output_dir`，不一致时会提示并阻止启动。
