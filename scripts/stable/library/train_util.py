...
def sample_images_common(
    ...
    if steps == 0:
        ...
        if args.sample_every_n_epochs is not None:
            ...
            if epoch is None or epoch % args.sample_every_n_epochs != 0:
                return
            # Keep epoch-based sampling aligned with epoch-based checkpoint behavior:
            # no extra sample on the final step boundary of this run/phase.
            if steps is not None and getattr(args, "max_train_steps", None) is not None:
                if int(steps) >= int(args.max_train_steps):
                    return
        else:
    ...
    try:
        ...
        if args.sample_prompts.endswith(".txt"):
            ...
            logger.error(f"Unsupported prompt file format / 未対応のプロンプト形式: {args.sample_prompts}")
            return
...
def sample_image_inference(
    ...
    try:
        ...
        if controlnet_image is not None:
            opened_controlnet_image = Image.open(controlnet_image).convert("RGB")
            controlnet_image = opened_controlnet_image.resize((width, height), Image.LANCZOS)
        ...
        if "out of memory" in error_text or "cudnn_status_alloc_failed" in error_text:
            logger.warning(
                f"[sample] OOM detected while generating sample image, skip this sample and continue training "
                f"(epoch={epoch}, steps={steps}, prompt_index={i})"
            )
            return
        raise
    finally:
        if opened_controlnet_image is not None:
            try:
                opened_controlnet_image.close()
            except Exception:
                pass
